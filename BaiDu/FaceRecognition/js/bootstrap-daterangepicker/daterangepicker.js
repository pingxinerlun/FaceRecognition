                ...
                '<div class="calendar left">' +
                    '<div class="daterangepicker_input">' +
                      ...
                      '<div class="calendar-time">' +
                        '<div></div>' +
                        '<i class="fa fa-clock-o-o glyphicon glyphicon-time"></i>' +
                      '</div>' +
                    '</div>' +
    ...
    DateRangePicker.prototype = {
        ...
        renderTimePicker: function (side) {
            ...
            if (side == 'left') {
                ...
                /*  原插件Bug:
                *       在开启时间选择的情况下，如果本次更改的左边时间大于上次选择的右边时间，
                *       会导致右边的分钟minute全部被disable掉，render时右边时间和日期都会变成NaN。

                *   修改方案：

                *       如果右边时间不在最小时间之后，则将右边的时间设置为minDate 加 timePickerIncrement
                *   张政权修改 2015-07-29 start
                */
                if (!selected.isAfter(minDate)) {
                    selected = minDate.clone().add(this.timePickerIncrement, this.timePickerSeconds ? 'seconds' : 'minutes');
                    this.setEndDate(selected);
                }
                /*
                * 张政权修改 2015-07-29 end
                */
            }
            ...
            for (var i = 0; i < 60; i += this.timePickerIncrement) {
                ...
                var disabled = false;
                //to make sure end time after start 2015-07-29 
                if (minDate && time.subtract(1,'minutes').second(59).isBefore(minDate))
                    disabled = true;
                if (maxDate && time.add(2, 'minutes').second(0).isAfter(maxDate))
                    disabled = true;
